...
export default function MainLogo(){
    ...
    return(
        <div onClick={()=>navigate("/")} className="flex items-center space-x-2 hover:cursor-pointer">
            <img src={companyInfoResponse?.data.info.logo} alt="Company Logo" className="w-14 h-14 rounded-full object-cover"/>
            <div className="flex flex-col items-center font-semibold gap-[0.2px] hidden lg:block">
                <h2 className="flex flex-row text-2xl font-semibold text-primary-400">{companyInfoResponse?.data.info.companyName}</h2>
